...
import math
import sqlite3
from constants import RAW_CLEANED_DATA_FILE_PATH, RAW_DATA_FILE_PATH
import psycopg2

from sqlalchemy import create_engine
#dialect+driver://username:password@host:port/database

engine=create_engine("postgresql+psycopg2://postgres:1234@localhost:5432/football")
...
def save_database_result(predict_file):
    ...
    prediction_results_df = pd.read_csv(predict_file)
    prediction_results_df = prediction_results_df[["Date", "HomeTeam", "AwayTeam", "FTHG", "FTAG", "FTR","prob_A","prob_D","prob_H"]]
    # prediction_results_df = prediction_results_df.loc[(prediction_results_df['FTHG'] == 0) | 
    #                         (prediction_results_df['FTAG'] == 0) | 
    #                         ((prediction_results_df['FTR'] == 'A')  & 
    #                             (prediction_results_df['FTR'] == 'H'))]
    prediction_results_df = prediction_results_df.loc[(prediction_results_df['prob_A'] >= 0) |
                                                    (prediction_results_df['prob_D'] >= 0) | (prediction_results_df['prob_H'] >= 0)] 
    try:
        prediction_results_df.to_sql('prediction_results', engine, if_exists= 'replace', index= True)
...
def save_datbase_standing_table(table_file):
    postgre = psycopg2.connect(
    database = 'football',
    user = 'postgres',
    password = '1234',
    ...
    port = '5432')
    cursor = postgre.cursor()
    prediction_results_df = pd.read_csv(table_file)
    prediction_results_df = prediction_results_df[["Team","Points","Goal_Diff","Win_Rate"]]
    try:
        prediction_results_df.to_sql('standing_table', engine, if_exists= 'replace', index= True)
...
if __name__ == "__main__":
    clean_all(RAW_DATA_FILE_PATH, RAW_CLEANED_DATA_FILE_PATH, 2006, 2020)