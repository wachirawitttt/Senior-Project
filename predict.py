...
from constants import FINAL_FILE
ipy = get_ipython()
if ipy is not None:
...
def get_clf(final_file_path, model_confidence_csv_path, clf_file, recalculate=True):
    ...
    data = data.loc[(data['FTR'] == 'H') | (
        data['FTR'] == 'D') | (data['FTR'] == 'A')]
    ...
    # MLPClassifier
    # clf_L = MLPClassifier()
    # parameters_L = {'hidden_layer_sizes': [(50, 50, 50), (50, 100, 50), (100,)],
    ...
    #     print(
    #         "Processing {}/{} of KFold Cross Validation...".format(split_index + 1, split))
    ...
    for index, clf in enumerate(classifiers):
        # print("KFold: {}/{}. clf_index: {}/{}.".format(split_index +
        #                                                1, split, index + 1, len(classifiers)))
...
def predict_next_round(clf, final_path, current_raw_cleaned_path, statistics=False, stat_path=None, first=True):
    ...
    if len(df) > 0:
        ...
        for (index, result, pred_prob) in zip(df_indices, prediction, prediction_probability):
            ...
            print("{:20} {:20} {:20} {}".format(
                HT, AT, HT if result == "H" else AT, max(pred_prob)))
